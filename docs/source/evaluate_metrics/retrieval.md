...
## 7. RAGAS Context Precision

### 📌Definition

Context Precision is a metric that evaluates whether all ground-truth relevant items present in the contexts are ranked
higher or not. Ideally, all relevant chunks should appear at the top ranks. This metric is computed using the question,
ground truth, and the contexts. Values range between 0 and 1, where higher scores indicate better precision.
For more information, you can see the
official [RAGAS docs](https://docs.ragas.io/en/latest/concepts/metrics/context_precision.html).
...
In this example, the relevant information about France being in Western Europe and Paris being the capital is
prominently present in the first chunk, making it a high context precision example.
...
"France, in Western Europe, encompasses medieval cities, alpine villages and Mediterranean beaches. Paris, its capital,
is famed for its fashion houses, classical art museums including the Louvre and monuments like the Eiffel Tower."
In this example, while the relevant information is present, it is not prominently ranked, making it a low context
precision example.

### ❗Additional Information

- **No Retrieval Ground Truth Required**: This metric does not require retrieval ground truth data, making it useful in
  scenarios where such data is unavailable.

- **Resource Intensive**: Using this metric with models like GPT-4 or GPT-4-turbo can be super expensive due to the high
  computational cost.

```{warning}
RAGAS context precision metric can cause **really expensive** API cost. 
The default model is gpt-3.5-turbo, but be sure you have proper cost limit on your OpenAI account.
```
