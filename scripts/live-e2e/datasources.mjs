...
const NATIVE_LANES = Object.freeze([
	{ name: "katok", binary: "katok", command: "scripts/manual-qa/run-qa-katok-live.ts" },
	{ name: "discrawl", binary: "discrawl", command: "scripts/manual-qa/run-qa-discrawl-live.ts" },
	...
	{ name: "mailcrawl", binary: "mailcrawl", command: "scripts/manual-qa/run-qa-mailcrawl-live.ts" },
	{ name: "spotlight", binary: "mdfind", command: "scripts/manual-qa/run-qa-spotlight-live.ts" },
]);
...
export function validateNativeIdentity(source) {
	return typeof source === "string" && !source.startsWith("/") && /^[a-z][a-z0-9-]*:.+/u.test(source);
}
...
async function runNativeLane(lane, root, which, configured, run) {
	...
	const result = await run("bun", [lane.command], process.cwd());
	if (result.ok) return { name: lane.name, status: "PASS", reason: "native-store harness passed", evidence: { binary: lane.binary, nativeStorePreserved: true, output: sanitizeDiagnostic(result.stdout).slice(-2000) } };
