...
async function worker(workspace: string, port: number): Promise<void> {
	for (const name of ["always.txt", "peers.md", "peers.bin", "never.txt", "does-not-exist.txt"]) wireSourceId(`/${ROOT_NAME}/${name}`);
	...
	const server = await startP2pServer({
		...
		workspaceRoots: [join(workspace, ROOT_NAME)],
		resolvePolicy,
		agent: {
			remoteSession: true,
			searchDocuments: async (query, options) => {
				((options as RetrievalOptionsWithObserved).observedSources)?.add(`/${ROOT_NAME}/always.txt`);
				return responseFor(query);
			},
		},
		injectionClassifier: false,
	});
	jsonLine({ ready: true, fingerprint: identity.fingerprint, pairingCode: createPairingCode(identity, `${HOST}:${port}`, `qa-${port}`) });
...
function spawnWorker(workspace: string, port: number): Child { return spawn(process.execPath, [SCRIPT, "--worker", workspace, String(port)], { cwd: resolve("."), stdio: ["ignore", "pipe", "inherit"], env: { ...process.env, OPENAI_API_KEY: "" } }) as Child; }
...
async function main(): Promise<void> {
	...
	try {
		...
		const a = spawnWorker(workspaceA, PORT_A); const b = spawnWorker(workspaceB, PORT_B); children.push(a, b); const [readyA, readyB] = await Promise.all([waitReady(a), waitReady(b)]); const check = (name: string, condition: unknown) => { assert(condition, `${name}: assertion failed`); passed++; log(`PASS ${name}`); };
		check(`instance A ready on ${PORT_A}`, readyA.ready === true); check(`instance B ready on ${PORT_B}`, readyB.ready === true); check("pairing codes accepted", loadPeerRegistry(workspaceA)["instance-b"]?.fingerprint === identityB.fingerprint && loadPeerRegistry(workspaceB)["instance-a"]?.fingerprint === identityA.fingerprint);
...
const args = process.argv.slice(2); if (args[0] === "--worker") await worker(String(args[1]), Number(args[2])); else { try { await main(); } catch (error) { process.stderr.write(`FAIL ${error instanceof Error ? error.message : String(error)}\n`); process.exitCode = 1; } }
