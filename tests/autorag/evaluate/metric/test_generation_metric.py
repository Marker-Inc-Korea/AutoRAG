...
def test_g_eval_fluency():
    scores = g_eval(
        similarity_generation_metric_inputs, model="gpt-3.5-turbo", metrics=["fluency"]
    )
    assert len(scores) == len(similarity_generation_metric_inputs)
    assert all(isinstance(score, float) for score in scores)


@pytest.mark.skipif(is_github_action(), reason="Skipping this test on GitHub Actions")
def test_g_eval_full():
    scores = g_eval(similarity_generation_metric_inputs, model="gpt-4o-mini")
    assert len(scores) == len(similarity_generation_metric_inputs)
    assert all(isinstance(score, float) for score in scores)
