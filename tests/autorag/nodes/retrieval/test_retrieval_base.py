...
def test_evenly_distribute_passages():
	...
	contents = [[f"test-{i}-{j}" for i in range(10)] for j in range(3)]
	top_k = 10
	new_ids, new_scores, new_content = evenly_distribute_passages(ids, scores, contents, top_k)
	assert len(new_ids) == top_k
	assert len(new_scores) == top_k
	assert new_scores == [0, 1, 2, 3, 0, 1, 2, 0, 1, 2]
	assert len(new_content) == top_k
