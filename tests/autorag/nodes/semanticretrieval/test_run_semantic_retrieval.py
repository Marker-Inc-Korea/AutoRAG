...
from autorag.nodes.semanticretrieval.run import run_semantic_retrieval_node
...
def test_run_semantic_retrieval_node(node_line_dir):
    modules = [VectorDB]
    module_params = [
    ...
    best_result = run_semantic_retrieval_node(
        modules, module_params, previous_result, node_line_dir, strategies
    )
    assert os.path.exists(os.path.join(node_line_dir, "semantic_retrieval"))
    expect_columns = [
        "qid",
        "query",
        "retrieval_gt",
    ...
    summary_path = os.path.join(node_line_dir, "semantic_retrieval", "summary.csv")
    vectordb_top_k_path = os.path.join(node_line_dir, "semantic_retrieval", "0.parquet")
    ...
    best_path = os.path.join(
        node_line_dir, "semantic_retrieval", f"best_{best_filename}"
    )
