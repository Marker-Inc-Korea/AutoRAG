...
def test_get_module_node_params():
    modules = [
        Module.from_dict({'module_type': 'bm25'}),
        Module.from_dict({'module_type': 'bm25', 'key2': 'value2'})
    ]
    node = Node(node_type='retrieval', strategy={'strategy_key': 'strategy_value'}, node_params={'param1': 'value1'},
                modules=modules)

    module_node_params = node.get_module_node_params()

    assert isinstance(module_node_params, list)
    assert module_node_params == [{'param1': 'value1'}, {'param1': 'value1', 'key2': 'value2'}]
