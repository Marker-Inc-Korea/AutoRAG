...
def get_new_trial_dir(history_df: pd.DataFrame, trial_config: TrialConfig,
				   trials_dir: str):
	trial_rows = history_df[history_df['trial_id'] == trial_config.trial_id]
	duplicate_corpus_rows = trial_rows[trial_rows['corpus_path'] == trial_config.corpus_path]
	...
	duplicate_qa_rows = duplicate_corpus_rows[trial_rows['qa_path'] == trial_config.qa_path]
	if len(duplicate_qa_rows) == 0: # If qa data changed
		...
		shutil.copy(trial_config.qa_path, os.path.join(existing_project_dir, "data", "qa.parquet"))
	duplicate_config_rows = duplicate_qa_rows[trial_rows['config_path'] == trial_config.config_path]
	...
	existing_project_dir = os.path.dirname(duplicate_qa_rows.iloc[0]['save_path'])
	latest_trial_name = get_latest_trial(os.path.join(existing_project_dir, "trial.json"))
