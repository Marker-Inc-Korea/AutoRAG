...
from src.run import run_parser_start_parsing, run_chunker_start_chunking, run_qa_creation, run_start_trial, \
	run_validate, run_dashboard, run_chat
from src.schema import ChunkRequest, ParseRequest, EnvVariableRequest, QACreationRequest, Project, Task, Status, \
	TaskType, TrialCreateRequest, Trial, TrialConfig
...
async def create_project():
	...
	if not os.path.exists(new_project_dir):
		...
		return jsonify({
			'error': f'Project name already exists: {data["name"]}'
		}), 400
	...
	for item in Path(WORK_DIR).iterdir():
		if item.is_dir():
			directories.append({
				...
				"path": str(item),
				"last_modified_datetime": datetime.fromtimestamp(item.stat().st_mtime),
...
async def get_trial_lists(project_id: str):
	...
	return jsonify({
		"total": len(trial_ids),
		"data": list(map(lambda x: trial_config_db.get_trial(x).model_dump(),
						 trial_ids)),
	})
...
async def create_new_trial(project_id: str):
	...
	if request_dict["config"] is not None:
		config_path = os.path.join(WORK_DIR, project_id, "config", f"{str(uuid.uuid4())}.yaml")
...
async def get_artifact_file(project_id: str):
    ...
    try:
        ...
        # 경로 검증 (디렉토리 트래버설 방지)
        if not os.path.normpath(full_path).startswith(os.path.normpath(os.path.join(WORK_DIR, project_id))):
            return jsonify({
...
@project_exists(WORK_DIR)
@trial_exists(WORK_DIR)
async def upload_files(project_id: str, trial_id: str):
	try:
		...
		upload_dir = os.path.join(WORK_DIR, project_id, "uploads", trial_id)
...
async def start_parsing(project_id: str, trial_id: str):
    ...
    try:
        ...
        logger.info(f"Creating task with data_path_glob: {data_path_glob}, project_dir: {save_path}, yaml_path: {yaml_path}")
        await create_task(task_id, task, run_parser_start_parsing, data_path_glob, save_path, yaml_path)
...
@app.route("/projects/<string:project_id>/trials/<string:trial_id>/chunk", methods=["POST"])
@project_exists(WORK_DIR)
@trial_exists(WORK_DIR)
async def start_chunking(project_id: str, trial_id: str):
	try:
		...
		parsed_data_path = os.path.join(WORK_DIR, project_id, "parse", f"parse_{previous_config.id}/0.parquet")
		# parsed_data_path 확인
		...
		await create_task(task_id, response, run_chunker_start_chunking,
...
@app.route("/projects/<string:project_id>/trials/<string:trial_id>/qa", methods=["POST"])
@project_exists(WORK_DIR)
@trial_exists(WORK_DIR)
async def create_qa(project_id: str, trial_id: str):
	...
	try:
		...
		corpus_filepath = os.path.join(WORK_DIR, project_id, "chunk", f"chunk_{previous_config.id}/0.parquet")
		...
		if corpus_filepath is None or not corpus_filepath or not os.path.exists(corpus_filepath):
			return jsonify({
				"error": "Corpus data path not found"
			}), 400
		...
		await create_task(task_id, response, run_qa_creation, qa_creation_request, corpus_filepath, dataset_dir)
		...
		return jsonify({
			"status": "error",
			"message": f"Failed at creation of QA: {str(e)}"
		}), 400


@app.route("/projects/<string:project_id>/trials/<string:trial_id>/config", methods=["GET"])
@project_exists(WORK_DIR)
@trial_exists(WORK_DIR)
...
@app.route("/projects/<string:project_id>/trials/<string:trial_id>/config", methods=["POST"])
@project_exists(WORK_DIR)
@trial_exists(WORK_DIR)
async def set_trial_config(project_id: str, trial_id: str):
	...
	if data.get("config", None) is not None:
		new_config_path = os.path.join(WORK_DIR, project_id, "config", f"{str(uuid.uuid4())}.yaml")
...
@app.route("/projects/<string:project_id>/trials/<string:trial_id>/validate", methods=["POST"])
@project_exists(WORK_DIR)
async def start_validate(project_id: str, trial_id: str):
	...
	await create_task(task_id, response,
					  run_validate, trial_config.qa_path, trial_config.corpus_path,
					  trial_config.config_path)

	return jsonify(response.model_dump()), 202


@app.route("/projects/<string:project_id>/trials/<string:trial_id>/evaluate", methods=["POST"])
@project_exists(WORK_DIR)
async def start_evaluate(project_id: str, trial_id: str):
	...
	if not os.path.exists(evaluate_history_path):
		evaluate_history_df = pd.DataFrame(columns=["trial_id", "save_dir", "corpus_path", "qa_path", "config_path"]) # save_dir is to autorag trial directory
	...
	await create_task(task_id, task,
					  run_start_trial, trial.config.qa_path, trial.config.corpus_path,
					  os.path.dirname(new_trial_dir),
					  trial.config.config_path, skip_validation, full_ingest)

	task.model_dump()
	return jsonify(task.model_dump()), 202


@app.route('/projects/<string:project_id>/trials/<string:trial_id>/report/open', methods=['GET'])
...
@app.route('/projects/<string:project_id>/trials/<string:trial_id>/report/close', methods=['GET'])
...
@app.route('/projects/<string:project_id>/trials/<string:trial_id>/chat/open', methods=['GET'])
...
async def get_project_artifacts(project_id: str):
    ...
    try:
        ...
        print(e)
        return jsonify({
            "error": f"Failed to scan artifacts: {str(e)}"
        }), 500
	
@app.route('/projects/<string:project_id>/trials/<string:trial_id>/chat/close', methods=['GET'])
...
async def get_tasks(project_id: str):
	if not os.path.exists(os.path.join(WORK_DIR, project_id)):
		return jsonify({
			'error': f'Project name does not exist: {project_id}'
		}), 404
	...
	return jsonify({
		...
		"data": evaluate_history_df.to_dict(orient='records')  # Convert DataFrame to list of dictionaries
	}), 200
...
async def get_task(project_id: str, task_id: str):
	...
	if task is None:
		return jsonify({
			'error': f'Task ID does not exist: {task_id}'
		}), 404
	response = task["task"]
