...
def test_set_and_get_trial(project_db, sample_trial):
    ...
    assert (
        retrieved_trial.config.model_dump() == sample_trial.config.model_dump()
    ), "Config 데이터가 일치해야 함"
...
def test_get_all_configs(project_db, sample_trial):
    project_db.set_trial(sample_trial)
    ...
    configs = project_db.get_all_configs()
    assert len(configs) == 1
    assert all([isinstance(config, TrialConfig) for config in configs])
    assert configs[0].corpus_path == "/path/to/corpus"
