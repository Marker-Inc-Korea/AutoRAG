...
TEST_HEADERS = {"Authorization": "Bearer good"}
...
def app_config():
    ...
    yield {"original_work_dir": original_work_dir, "test_work_dir": test_work_dir}
...
async def test_create_project(test_client, app):
    ...
    response = await test_client.post(
        "/projects", json=project_data, headers=TEST_HEADERS
    )
    ...
    ), f"Expected description: {project_data['description']}, got: {content}"
...
async def test_create_trial_with_tasks(test_client, app):
    ...
    trial_response = await test_client.post(
        f"/projects/{project_data_name}/trials", json=trial_data, headers=TEST_HEADERS
    )
...
async def test_create_trial(test_client, app):
    ...
    assert (
        status_code == 200
    ), f"Expected 200, got {status_code}. Response: {response_data}"
    ...
    # Trial DB 파일 검증
    trial_db_path = (
        Path(app.config["AUTORAG_WORK_DIR"]) / project_data_name / "trials.db"
    )
...
async def test_create_trial_invalid_project(test_client):
    ...
    response = await test_client.post(
        "/projects/nonexistent_project/trials", json=trial_data, headers=TEST_HEADERS
    )
...
async def test_get_trial(test_client):
    ...
    await test_client.post(
        "/projects", json={"name": project_data_name}, headers=TEST_HEADERS
    )
    ...
    response = await test_client.get(
        f"/projects/{project_data_name}/trials/{trial_id}", headers=TEST_HEADERS
    )
