	...
	skillManifest(): DatasourceSkillManifest {
		return {
			...
			content: [
				...
				"",
				"## Indexing",
				"Indexing is incremental: refresh runs `clawgallery bootstrap`, then `clawgallery vdr sync` when configured. Do not trigger captioning or renaming from AutoRAG.",
				"",
				"## How to search",
				"Call `search_datasource_documents` with a natural-language query and `topK`.",
				"",
				"## Native CLI",
				"You can also invoke `clawgallery` directly through `bash` when you need its full surface:",
				"- `clawgallery search keyword <query> --json` — path/caption keyword search",
				"- `clawgallery search hybrid <query> --json` — combined retrieval",
				"- `clawgallery vdr status --json` — index capability check",
				"- `clawgallery --help` — full command reference",
				"",
				"## Configuration",
				"clawgallery uses its own native config by default. Server-side connector overrides: `path` → `clawgallery --path <dir>` (image root), `configDir` → `CLAWGALLERY_CONFIG_DIR`, `vdrBackend` (dense/sparse index backend), `binaryPath`.",
				"",
				"## Search mode rules",
				"",
