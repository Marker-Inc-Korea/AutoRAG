	...
	skillManifest(): DatasourceSkillManifest {
		return {
			...
			content: [
				...
				"",
				`Hybrid retrieval is the default. Use \`search_datasource_documents\` with a natural-language query and optionally narrow \`scope\` to an authorized \`/${CLAWGALLERY_SOURCE_KIND}/<instance>/**\`.`,
			].join("\n"),
		};
	}
}
