...
function createCloudDriveSkillDefinition(skillName: string, provider?: string): ConnectorSkillDefinition {
	...
	return {
		...
		manifestNotes: [
			...
			`This connection is independently addressable. Load \`datasource-${skillName}\`, then call \`${datasourceSearchToolName(skillName)}\` with an optional narrowing scope under \`/${skillName}/<instance>/**\`.`,
		],
		nativeCliNote:
