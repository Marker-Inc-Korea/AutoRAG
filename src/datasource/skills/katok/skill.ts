	...
	skillManifest(): DatasourceSkillManifest {
		...
		return {
			...
			content: [
				...
				"",
				"## Configuration",
				"katok uses its own native store (`~/Library/Application Support/katok` on macOS) by default; AutoRAG never creates or manages it.",
				"Server-side connector overrides: `workspacePath` → `katok --data-dir <dir>`, `configPath` → `katok --config <file>`, `binaryPath` → explicit katok binary.",
				"",
