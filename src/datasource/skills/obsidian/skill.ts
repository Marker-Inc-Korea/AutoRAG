	...
	skillManifest(): DatasourceSkillManifest {
		...
		return {
			...
			content: [
				...
				"",
				"## Configuration",
				"`vaultPath` (server config, required) points at the Obsidian vault. qmd uses its own native config and cache by default.",
				"Only when the operator supplies `configPath` does AutoRAG write the vault collection definition (`index.yml`) there and export it as `QMD_CONFIG_DIR`; `binaryPath` selects an explicit qmd binary.",
				"",
				"## Output rules",
