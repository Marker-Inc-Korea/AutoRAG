	...
	if any([module.__name__ in hybrid_module_names for module in modules]):
		...
		if all(
			...
			best_semantic_summary_row = semantic_summary_df.loc[
				semantic_summary_df["is_best"] == True
			].iloc[0]
			best_lexical_summary_row = lexical_summary_df.loc[
				lexical_summary_df["is_best"] == True
			].iloc[0]
			target_modules = (
		...
		hybrid_selected_filename, hybrid_summary_df, hybrid_results, hybrid_times = (
			None,
			pd.DataFrame(),
			[],
			[],
...
def get_hybrid_execution_times(lexical_summary, semantic_summary) -> float:
	lexical_execution_time = lexical_summary.loc[
		lexical_summary["is_best"] == True
	].iloc[0]["execution_time"]
	semantic_execution_time = semantic_summary.loc[
		semantic_summary["is_best"] == True
	].iloc[0]["execution_time"]
