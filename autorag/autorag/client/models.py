...
class RetrievalResults:
    def __init__(self, retrievals: list, **kwargs):
        ...
        self.retrievals = [
            r if isinstance(r, Retrieval) else Retrieval(**r)
            for r in retrievals
        ]
        for key, value in kwargs.items():
            setattr(self, key, value)
    ...
    def __str__(self) -> str:
        ...
        return "\n".join(f"- {r.text} (score: {r.score}, source: {r.metadata.get('doc_id', 'unknown doc')}::chunk_{r.metadata.get('chunk_id', 'unknown chunk_id')})" 
