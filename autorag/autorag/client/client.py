...
class AutoRAGClient:
    ...
    async def embedding(self, project_id: str, vector_storage: str = "auto"):
        ...
        await self._post(f"/projects/{project_id}/embedding", {
            "vector_storage": vector_storage
        })

    async def create_rag_pipeline(self, project_id: str, embedding_model: str = "auto") -> RAGPipeline:
        """Create a RAG pipeline"""
        if project_id not in self._projects:
            raise APIError(f"Project {project_id} not found")
        
        try:
            await self._post(f"/projects/{project_id}/pipeline", {
    ...
    async def query_rag(self, rag_pipeline, question: str):
        ...
        try:
            response = await self._post(
                f"/projects/{rag_pipeline.project_id}/query",
                {"question": question}
            )
            ...
            return type('QueryResponse', (), {
                ...
                'retrievals': [
                    Retrieval(**r) for r in response['retrievals']
                ]
            })()
        finally:
