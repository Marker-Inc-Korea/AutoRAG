...
describe("renderMemoryContext", () => {
	...
	it("renders single entry as markdown table", () => {
		const entries = [makeEntry({ query: "find files", method: "posix", outcome: "success" })];
	...
	it("groups multiple entries for same query", () => {
		const entries = [
			makeEntry({ query: "search code", method: "posix", outcome: "success" }),
			makeEntry({ query: "search code", method: "posix", outcome: "success" }),
			makeEntry({ query: "search code", method: "posix", outcome: "failure" }),
		];
		const result = renderMemoryContext(entries);
	...
	it("renders multiple methods for same query as separate rows", () => {
		const entries = [
			makeEntry({ query: "find docs", method: "posix", outcome: "success" }),
			makeEntry({ query: "find docs", method: "vector", outcome: "failure" }),
		];
		const result = renderMemoryContext(entries);
	...
	it("sorts by most recent activity", () => {
		const old = makeEntry({ query: "old query", method: "posix", outcome: "success", timestamp: 1000 });
		const recent = makeEntry({ query: "new query", method: "posix", outcome: "success", timestamp: 9000 });
	...
	it("caps at maxGroups", () => {
		...
		for (let i = 0; i < 60; i++) {
			entries.push(makeEntry({ query: `query-${i}`, method: "posix", outcome: "success", timestamp: i }));
		}
	...
	it("formats date correctly", () => {
		...
		const entries = [makeEntry({ query: "test", method: "posix", outcome: "success", timestamp: ts })];
		const result = renderMemoryContext(entries);
