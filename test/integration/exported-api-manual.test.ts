...
// here. Assertions focus on the real exported contract: path opacity,
...
describe("exported API — #6 curated output path opacity", () => {
	it("redacts real paths from the root-exported searchDocuments response and keeps the registry intact", async () => {
		...
		expect(blob).not.toContain("/docs/report.txt");
...
describe("exported API — #21 structured degraded-mode diagnostics", () => {
	it("returns a path-free minsync-unavailable diagnostic via the root-exported retrieveWithDiagnostics", async () => {
...
describe("exported API — #22 observable refresh status and watch", () => {
	...
	it("keeps parsed mirrors current via the root-exported startWatchRefresh and stops cleanly", async () => {
		...
		const parsedRoot = join(root, ".autorag", "parsed");
		const start = Date.now();
		let updated = false;
		while (Date.now() - start < 4000) {
			const status = await agent.getRefreshStatus();
			if (!status.stale && (status.counts?.written ?? 0) >= 1) {
				updated = true;
				break;
			}
		...
		expect(existsSync(parsedRoot)).toBe(true);
	});
});
