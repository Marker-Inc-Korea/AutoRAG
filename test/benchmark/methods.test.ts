...
async function importMethodsWithFsInstrumentation(instrumentation: {
	...
	readonly onOpenDirectory?: (path: unknown) => void;
	...
	vi.doMock("node:fs/promises", async () => {
		...
		return {
			...
			opendir: ((...args: unknown[]) => {
				instrumentation.onOpenDirectory?.(args[0]);
				return Reflect.apply(actual.opendir, actual, args);
...
function writeFakeMinSync(root: string): { binaryPath: string; queryStatePath: string; syncStatePath: string } {
	...
	writeFileSync(
		binaryPath,
		`#!/usr/bin/env node
const { mkdirSync, readFileSync, statSync, symlinkSync, utimesSync, writeFileSync } = require("node:fs");
...
if (args[0] === "init") {
  ...
  if (state.generatedConfigSymlinkTarget) {
    symlinkSync(state.generatedConfigSymlinkTarget, configPath);
  } else if (state.generatedConfigBytes) {
    writeFileSync(configPath, " ".repeat(state.generatedConfigBytes), { mode: 0o600 });
  } else {
...
if (args[0] === "sync") {
  const state = JSON.parse(readFileSync(${JSON.stringify(syncStatePath)}, "utf8"));
  if (state.syncMarkerPath) writeFileSync(state.syncMarkerPath, "sync-called");
  ...
  if (state.collectionEntries) {
    const directory = join(process.cwd(), ".minsync", "store");
    for (let index = 0; index < state.collectionEntries; index += 1) {
      writeFileSync(join(directory, \`entry-\${String(index).padStart(5, "0")}.lance\`), "");
    }
  }
...
`;
	...
	it("rejects an oversized config generated by external MinSync init before synchronization", async () => {
		const parent = makeRoot();
		const workspace = materializeBenchmarkWorkspace(join(parent, "workspace"), [
			{ documentId: "doc", title: "제목", text: "query" },
		]);
		const { binaryPath, syncStatePath } = writeFakeMinSync(parent);
		const syncMarkerPath = join(parent, "sync-called");
		writeFileSync(
			syncStatePath,
			JSON.stringify({
				ok: true,
				stdout: JSON.stringify({ synced: 1 }),
				generatedConfigBytes: 1024 * 1024 + 1,
				syncMarkerPath,
			}),
		);
	...
	it("does not rewrite an external target symlinked as the generated MinSync config", async () => {
		const parent = makeRoot();
		const workspace = materializeBenchmarkWorkspace(join(parent, "workspace"), [
			{ documentId: "doc", title: "제목", text: "query" },
		]);
		const { binaryPath, syncStatePath } = writeFakeMinSync(parent);
		const externalTarget = join(parent, "external-config.toml");
		const original = [
			"version = 1",
			"[collection]",
			'path = "store"',
		...
		writeFileSync(externalTarget, original, { mode: 0o600 });
		writeFileSync(
			syncStatePath,
			JSON.stringify({
				ok: true,
				stdout: JSON.stringify({ synced: 1 }),
				generatedConfigSymlinkTarget: externalTarget,
			}),
		);
	...
	it("streams directory enumeration and promptly rejects the 4097th integrity entry", async () => {
		const parent = makeRoot();
		const workspace = materializeBenchmarkWorkspace(join(parent, "workspace"), [
			{ documentId: "doc", title: "제목", text: "query" },
		]);
		...
		writeFileSync(
			...
			JSON.stringify({
				ok: true,
				stdout: JSON.stringify({ synced: 1 }),
				collectionEntries: 4_097,
			}),
		);
		const collectionPath = join(workspace.root, ".autorag", "minsync", ".minsync", "store");
		let collectionReaddirSyncCalls = 0;
		const instrumentedMethods = await importMethodsWithFsInstrumentation({
			onReadDirectory: (path) => {
				if (String(path) === collectionPath) collectionReaddirSyncCalls += 1;
			},
		});
		...
		expect(collectionReaddirSyncCalls).toBe(0);
